function plotar_resultados_tiro
...
%% Entrada de dados
arquivo = 'resultados_duffing_tiro*.txt';

load('duffing_tiro.mat', 'modelo', 'parametros');
tiro=modelo.data;
...
for i=1:size(all_data,1)
    x0=all_data(i, 1:2)';
    pars0=all_data(i,(end-tiro.p)+1:end)';
    if tiro.autonomo==false
        fun=@(tt,xx) tiro.din_func(tt,xx,pars0);
    else
        fun=@(tt,xx) tiro.din_func(xx,pars0);
    end
    [t,y]=ode45(fun, linspace(0,1,200), x0);
    new_data(i,:)=[max(y,[],1), pars0'];
end
all_data=new_data 
